class Netstat {
    constructor(parentId) {
        ...
        let geolite2 = window.geolite2Redist;
        let maxmind = require("maxmind");
        let geoIPcache = require("path").join(require("@electron/remote").app.getPath("userData"), "geoIPcache");
        geolite2.downloadDbs({path: geoIPcache}).then(() => {
           geolite2.open('GeoLite2-City', path => {
                return maxmind.open(path);
            }, geoIPcache).catch(e => {throw e}).then(lookup => {
