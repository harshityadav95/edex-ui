...
function preloadRendererDependencies() {
    const colorModule = require("color");
    const prettyBytesModule = require("pretty-bytes");
    const usernameModule = require("username");
    const pdfjsModule = require("./node_modules/pdfjs-dist/legacy/build/pdf.mjs");
    const geolite2Module = require("./node_modules/geolite2-redist/dist/index.js");
    const ligaturesModule = require("./node_modules/@xterm/addon-ligatures/lib/addon-ligatures.mjs");

    window.Color = colorModule.default;
    window.prettyBytes = prettyBytesModule.default;
    window.getUsername = usernameModule.username;
    window.pdfjsLib = pdfjsModule;
    window.geolite2Redist = geolite2Module;
    window.LigaturesAddon = ligaturesModule.LigaturesAddon;
}

let i = 0;
try {
    preloadRendererDependencies();
    ...
    document.getElementById("boot_screen").innerHTML += `${error}<br/>Unable to preload renderer dependencies.`;
    throw error;
}
